...
# RESUME LOGIC REMOVED for fallback
start_group = None
...
for group_name in groups_to_scrape:
    if group_name == start_group or start_group is None:
    ...
    for idx, comm in enumerate(original_commodities):
        if (group_name == start_group and comm == start_commodity) or start_commodity is None:
        ...
        try:
            ...
            # From Date (Dynamic based on DB max date)
            db_conn = sqlite3.connect("data/scraped_prices.db")
            db_cursor = db_conn.cursor()
            db_cursor.execute("SELECT arrival_date FROM scraped_prices ORDER BY id DESC LIMIT 1")
            last_date_row = db_cursor.fetchone()
            db_conn.close()
            ...
            if last_date_row and last_date_row[0]:
                start_date = parse_date(last_date_row[0])
            else:
            ...
            month_select = Select(wait_comm.until(EC.presence_of_element_located((By.XPATH, f"//select[option[text()='{start_month_str}']]"))))
            month_select.select_by_visible_text(start_month_str)
            time.sleep(1)

            year_select = Select(wait_comm.until(EC.presence_of_element_located((By.XPATH, f"//select[option[text()='{start_year_str}']]"))))
            year_select.select_by_visible_text(start_year_str)
            time.sleep(1)

            day_1 = wait_comm.until(EC.element_to_be_clickable((By.XPATH, f"//div[contains(@class, 'grid-cols-7')]//div[text()='{start_day_str}' and not(contains(@class, 'opacity-40'))]")))
